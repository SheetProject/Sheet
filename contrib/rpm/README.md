...
The RPM spec file provided here is for Sheet-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://sheetproject.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Sheet git repository](https://github.com/SheetProject/Sheet) but are not part of
...
Currently only `Source10` is used, for the example `sheet.conf` file.
...
in the sheet git repository. At present only an SVG version of the Sheet
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the sheet
...
needed when 0.12.1 is released, a proper fix is already in the Sheet git
master branch.
...
idea to build Sheet Core against a static version of that library compiled
...
The `sheet.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `sheet-server` package. At this point, most RPM based
...
The `sheet-server` package should have SELinux support. How to properly do
...
Sheet-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/sheet/). He has been
packaging Sheet for Fedora at least since 2012.
...
4. I renamed the `sheet` package that contains the Qt GUI to `sheet-core` as
that appears to be how the general population refers to it, in contrast to
`sheet-xt` or `sheet-classic`. I wanted to make sure the general population
