...
void RestrictedAssetsDialog::setupStyling(const PlatformStyle *platformStyle)
{
    ...
    if (darkModeEnabled) {
        ui->labelAssetBalance->setStyleSheet(STRING_LABEL_COLOR_DARK);
        ui->labelAddressList->setStyleSheet(STRING_LABEL_COLOR_DARK);
    } else {
...
void RestrictedAssetsDialog::freezeAddressClicked()
{
    ...
    if(nRequiredFee > 0)
    {
        ...
        questionString.append(SheetUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), nRequiredFee));
