...
#include "sheetaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Sheet address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new SheetAddressEntryValidator(parent));
    widget->setCheckValidator(new SheetAddressCheckValidator(parent));
}
...
bool parseSheetURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no sheet: URI
    if(!uri.isValid() || uri.scheme() != QString("sheet"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!SheetUnits::parse(SheetUnits::SHTX, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseSheetURI(QString uri, SendCoinsRecipient *out)
{
    // Convert sheet:// to sheet:
    //
    //    Cannot handle this later, because sheet:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("sheet://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "sheet:");
    }
    QUrl uriInstance(uri);
    return parseSheetURI(uriInstance, out);
}

QString formatSheetURI(const SendCoinsRecipient &info)
{
    QString ret = QString("sheet:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(SheetUnits::format(SheetUnits::SHTX, info.amount, false, SheetUnits::separatorNever));
        paramCount++;
    }
...
bool openSheetConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(SHEET_CONF_FILENAME);
    ...
    /* Open sheet.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Sheet.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Sheet (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Sheet (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "sheet.desktop";
    return GetAutostartDir() / strprintf("sheet-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a sheet.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Sheet\n";
        else
            optionFile << strprintf("Name=Sheet (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the sheet app
...
bool GetStartOnSystemStartup()
{
    CFURLRef sheetAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (sheetAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, sheetAppUrl);

    CFRelease(sheetAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add sheet app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, sheetAppUrl, nullptr, nullptr);
    }
