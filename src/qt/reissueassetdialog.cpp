...
//        ui->optInRBF->setCheckState(model->getDefaultWalletRbf() ? Qt::Checked : Qt::Unchecked);
        ...
        ui->reissueCostLabel->setText(tr("Cost") + ": " + SheetUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::REISSUE)));
...
void ReissueAssetDialog::setupAssetDataView(const PlatformStyle *platformStyle)
{
    ...
    if (darkModeEnabled) {
        ...
        ui->unitLabel->setStyleSheet(STRING_LABEL_COLOR_DARK);
    ...
    if (darkModeEnabled) {
        ui->currentDataLabel->setStyleSheet(STRING_LABEL_COLOR_DARK);
        ui->reissueAssetDataLabel->setStyleSheet(STRING_LABEL_COLOR_DARK);
...
void ReissueAssetDialog::CheckFormState()
{
    ...
    if (!ui->addressText->text().isEmpty()) {
        if (!IsValidDestination(dest)) {
            showMessage(tr("Invalid Sheet Destination Address"));
            return;
        }
    }
...
void ReissueAssetDialog::onReissueAssetClicked()
{
    ...
    QString amount = "<b>" + QString::fromStdString(ValueFromAmountString(GetReissueAssetBurnAmount(), 8)) + " SHTX";
    amount.append("</b>");
