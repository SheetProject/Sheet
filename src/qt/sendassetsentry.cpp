    ...
    platformStyle(_platformStyle)
{
    ...
    // normal sheet address field
    GUIUtil::setupAddressWidget(ui->payTo, this);
    // just a label for displaying sheet address(es)
    ...
    if (darkModeEnabled) {
        ui->assetBoxLabel->setStyleSheet(STRING_LABEL_COLOR_DARK);
        ui->payToLabel->setStyleSheet(STRING_LABEL_COLOR_DARK);
        ui->labellLabel->setStyleSheet(STRING_LABEL_COLOR_DARK);
        ui->amountLabel->setStyleSheet(STRING_LABEL_COLOR_DARK);
        ui->messageLabel->setStyleSheet(STRING_LABEL_COLOR_DARK);
        ui->administratorCheckbox->setStyleSheet(QString(".QCheckBox{ %1; }").arg(STRING_LABEL_COLOR_DARK));
    } else {
    ...
    if (darkModeEnabled) {
        ui->memoLabel->setStyleSheet(STRING_LABEL_COLOR_DARK);
    } else {
