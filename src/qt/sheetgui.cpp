...
const std::string SheetGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString SheetGUI::DEFAULT_WALLET = "~Default";

SheetGUI::SheetGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void SheetGUI::createActions()
{
    ...
    overviewAction = new QAction(platformStyle->SingleColorIcon(QIcon(":/icons/overview"), platformStyle->ToolBarSelectedTextColor()), tr("&Overview"), this);
    ...
    sendCoinsAction = new QAction(platformStyle->SingleColorIcon(QIcon(":/icons/send"), platformStyle->ToolBarSelectedTextColor()), tr("&Send"), this);
    sendCoinsAction->setStatusTip(tr("Send coins to a Sheet address"));
    ...
    receiveCoinsAction = new QAction(platformStyle->SingleColorIcon(QIcon(":/icons/receiving_addresses"), platformStyle->ToolBarSelectedTextColor()), tr("&Receive"), this);
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and sheet: URIs)"));
    ...
    historyAction = new QAction(platformStyle->SingleColorIcon(QIcon(":/icons/history"), platformStyle->ToolBarSelectedTextColor()), tr("&Transactions"), this);
    ...
    /** SHTX START */
    transferAssetAction = new QAction(platformStyle->SingleColorIcon(QIcon(":/icons/asset_transfer"), platformStyle->ToolBarSelectedTextColor()), tr("&Transfer Assets"), this);
    transferAssetAction->setStatusTip(tr("Transfer assets to SHTX addresses"));
    ...
    createAssetAction = new QAction(platformStyle->SingleColorIcon(QIcon(":/icons/asset_create"), platformStyle->ToolBarSelectedTextColor()), tr("&Create Assets"), this);
    ...
    manageAssetAction = new QAction(platformStyle->SingleColorIcon(QIcon(":/icons/asset_manage"), platformStyle->ToolBarSelectedTextColor()), tr("&Manage Assets"), this);
    ...
    restrictedAssetAction = new QAction(platformStyle->SingleColorIcon(QIcon(":/icons/asset_restricted"), platformStyle->ToolBarSelectedTextColor()), tr("&Restricted Assets"), this);
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Sheet addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Sheet addresses"));
    ...
    openAction->setStatusTip(tr("Open a sheet: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Sheet command-line options").arg(tr(PACKAGE_NAME)));
...
void SheetGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/sheettext")));
        ...
        QVBoxLayout* sheetLabelLayout = new QVBoxLayout(toolbarWidget);
        sheetLabelLayout->addWidget(label);
        sheetLabelLayout->addWidget(toolbar);
        sheetLabelLayout->setDirection(QBoxLayout::TopToBottom);
        sheetLabelLayout->addStretch(1);
...
void SheetGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool SheetGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool SheetGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void SheetGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void SheetGUI::setWalletActionsEnabled(bool enabled)
{
...
void SheetGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void SheetGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void SheetGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void SheetGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void SheetGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void SheetGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void SheetGUI::openClicked()
{
...
void SheetGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void SheetGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void SheetGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void SheetGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void SheetGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void SheetGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** SHTX START */
void SheetGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void SheetGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void SheetGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void SheetGUI::gotoRestrictedAssetsPage()
{
...
void SheetGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Sheet network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void SheetGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void SheetGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void SheetGUI::updateHeadersSyncProgressLabel()
{
...
void SheetGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void SheetGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Sheet"); // default title
...
void SheetGUI::changeEvent(QEvent *e)
{
...
void SheetGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void SheetGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void SheetGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "SHTX")
        msg += tr("Amount: %1\n").arg(SheetUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(SheetUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void SheetGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void SheetGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool SheetGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool SheetGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void SheetGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void SheetGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void SheetGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void SheetGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void SheetGUI::detectShutdown()
{
...
void SheetGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void SheetGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void SheetGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(SheetGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
static bool ThreadSafeMnemonic(SheetGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void SheetGUI::subscribeToCoreSignals()
{
...
void SheetGUI::unsubscribeFromCoreSignals()
{
...
void SheetGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void SheetGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<SheetUnits::Unit> units = SheetUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const SheetUnits::Unit unit : units)
    {
    	#ifndef QTversionPreFiveEleven
        	max_width = qMax(max_width, fm.horizontalAdvance(SheetUnits::name(unit)));
        #else
        	max_width = qMax(max_width, fm.width(SheetUnits::name(unit)));
        #endif
    }
    ...
    if (darkModeEnabled) {
        setStyleSheet(QString("QLabel { color : %1; }").arg(STRING_LABEL_COLOR_DARK));
    } else {
        setStyleSheet(QString("QLabel { color : %1; }").arg(STRING_LABEL_COLOR));
    }
}
...
{
    menu = new QMenu(this);
    for (SheetUnits::Unit u : SheetUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(SheetUnits::name(u)), this);
...
{
    setText(SheetUnits::name(newUnits));
}
