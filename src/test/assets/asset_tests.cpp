...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of SHEETCOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("SHTX", type));
        BOOST_CHECK(!IsAssetNameValid("SHEET", type));
        BOOST_CHECK(!IsAssetNameValid("SHEETCOIN", type));

        //- Versions of SHEETCOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("SHEET.COIN", type));
        BOOST_CHECK(IsAssetNameValid("SHEET_COIN", type));
        BOOST_CHECK(IsAssetNameValid("SHTXSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERSHTX", type));
        BOOST_CHECK(IsAssetNameValid("SHEETSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDESHEET", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_SHEETS", type));
        BOOST_CHECK(IsAssetNameValid("SESHTXOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        CAssetTransfer asset("SHEET", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("SHEET", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(GetParams().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
