...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for SHTX
    ...
    if (p_rewardSnapshot.strDistributionAsset != "SHTX") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is SHTX with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using SHTX differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "SHTX") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Sheet address (already validated during ownership snapshot creation)
